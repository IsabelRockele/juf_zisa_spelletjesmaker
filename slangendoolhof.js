document.addEventListener("DOMContentLoaded", () => {
    ...
    function drawWorksheetHeader() {
        ...
        ctx.beginPath();
        roundRect(ctx, 48, 122, 150, 42, 12);
        roundRect(ctx, 602, 122, 150, 42, 12);
        ctx.fill();
        ctx.fillStyle = "#3478b8";
        ctx.font = "700 13px Arial";
        ctx.textAlign = "center";
        ctx.fillText("OEFENINGEN", 123, 143);
        ctx.fillText("ANTWOORDEN", 677, 143);

        ctx.strokeStyle = "#d4e0e9";
        ctx.setLineDash([5, 7]);
        ctx.beginPath();
        ctx.moveTo(48, 181); ctx.lineTo(752, 181);
        ctx.stroke();
        ctx.restore();
    }
    ...
    function generatePathWaypoints(startPoint, endPoint, allOtherPaths, pathIndex, pathCount) {
        ...
        const loop = {
            ...
            horizontalRadius: 66 + Math.random() * 18,
            verticalRadius: 56 + Math.random() * 18
        };
    ...
    function drawPath(waypoints, strokeStyle, lineWidth) {
        ...
        // Catmull-Rom naar Bézier: de lijn gaat door elk punt zonder scherpe
        // knikken of dichtgeknepen lusjes.
