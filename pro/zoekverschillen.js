document.addEventListener('DOMContentLoaded', () => {
    ...
    const autoDifferencesPanel=document.getElementById('autoDifferencesPanel'),makeAutoDifferencesBtn=document.getElementById('makeAutoDifferencesBtn'),differenceSummary=document.getElementById('differenceSummary'),differenceSummaryList=document.getElementById('differenceSummaryList');
    const solutionActions=document.getElementById('solutionActions'),toggleSolutionBtn=document.getElementById('toggleSolutionBtn'),downloadSolutionPdfBtn=document.getElementById('downloadSolutionPdfBtn');
    ...
    const solutionOverlayCanvas=document.getElementById('solutionOverlayCanvas'),solutionOverlayCtx=solutionOverlayCanvas.getContext('2d');
    ...
    const toolIcons = {
        ...
        lijn:'<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M5 19 19 5"/><circle cx="5" cy="19" r="2"/><circle cx="19" cy="5" r="2"/></svg>',cirkel:'<svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="12" cy="12" r="8.5"/></svg>',rechthoek:'<svg viewBox="0 0 24 24" aria-hidden="true"><rect x="4" y="5" width="16" height="14" rx="1.5"/></svg>',
        pipet:'<svg viewBox="0 0 24 24" aria-hidden="true"><path d="m14.5 5.5 4 4M8 16l8.8-8.8a2.1 2.1 0 1 0-3-3L4.7 13.3 4 18l4.7-.7Z"/><path d="M4 20h6"/></svg>',opvulemmer:'<svg viewBox="0 0 24 24" aria-hidden="true"><path d="m5 12 7-7 7 7-7 7-7-7Z"/><path d="m8.5 8.5 7 7M19.5 15.5s2 2.3 2 3.5a2 2 0 0 1-4 0c0-1.2 2-3.5 2-3.5Z"/></svg>',tekst:'<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M5 5h14M12 5v14M8.5 19h7"/></svg>'
    };
    ...
    const catalog = [
        ['naar-school','Naar school',['klas-met-kinderen','speelplaats']],['herfst','Herfst',['boswandeling','dieren-in-het-bos']],['sinterklaas','Sinterklaas',['pakjesavond','stoomboot']],['kerst','Kerst',['kerstkamer','wintermarkt']],['pasen','Pasen',['paaseieren-zoeken','paastuin']],['lente','Lente',['lentetuin','boerderij-in-de-lente']],['carnaval','Carnaval',['carnavalsstoet','verkleedfeest']],['winter','Winter',['sneeuwpret','winterdorp']],['zomer','Zomer',['stranddag','kamperen']],['valentijn','Valentijn',['vriendschapsfeest','hartjestuin']],['dieren','Dieren',['dierenpark','boerderijdieren']],['voertuigen','Voertuigen',['drukke-straat','bouwwerf']],['prehistorie','Prehistorie',['prehistorisch-kamp','grotschilderingen']],['oudheid','Oudheid',['romeinse-markt','egyptische-nijl']],['middeleeuwen','Middeleeuwen',['kasteelleven','middeleeuwse-markt']],['vroegmoderne-tijd','Vroegmoderne tijd',['ontdekkingsreis','drukkerij']],['moderne-tijd','Moderne tijd',['stoomtreinstation','oude-fabriek']],['hedendaagse-tijd','Hedendaagse tijd',['moderne-stad','duurzame-buurt']],['ruimte','Ruimte',['ruimtestation','maanverkenning']],['de-zee','De zee',['onderwaterwereld','aan-de-kust']],['sport','Sport',['sportdag','sporthal']]
    ];
    const toolInfo = {potlood:{name:'Potlood',text:'Teken vrij op de rechterafbeelding.'},gum:{name:'Gum',text:'Wis een deel van de rechterafbeelding.'},select:{name:'Selecteren',text:'Sleep een kader rond een onderdeel dat je wilt kopiëren of verplaatsen.'},lijn:{name:'Lijn',text:'Trek een rechte lijn in de rechterafbeelding.'},cirkel:{name:'Cirkel',text:'Sleep om een cirkel of ovaal te tekenen.'},rechthoek:{name:'Rechthoek',text:'Sleep om een rechthoek te tekenen.'},pipet:{name:'Kleur kiezen',text:'Klik in de afbeelding om exact die kleur over te nemen.'},opvulemmer:{name:'Opvullen',text:'Klik in een vlak om het met de gekozen kleur te vullen.'},tekst:{name:'Tekst',text:'Vul tekst voor links en rechts in en klik daarna op de gewenste plaats.'}};
    ...
    const automaticDifferencePoints={'klas-met-kinderen':[[.54,.22,.11],[.277,.16,.06],[.797,.235,.06],[.925,.282,.06],[.425,.675,.05],[.33,.47,.05],[.126,.90,.045],[.758,.92,.045],[.412,.525,.04],[.132,.188,.055]],'speelplaats':[[.203,.515,.055],[.785,.75,.05],[.886,.79,.05],[.506,.188,.045],[.69,.48,.07],[.596,.763,.055],[.073,.895,.04],[.328,.14,.035],[.50,.335,.04],[.804,.285,.04]],'dieren-in-het-bos':[[.58,.73,.075],[.78,.62,.07],[.76,.15,.065],[.58,.42,.09],[.10,.52,.08],[.39,.59,.06],[.70,.91,.06],[.72,.865,.055],[.12,.82,.065],[.12,.20,.075]],'boswandeling':[[.14,.48,.065],[.16,.87,.06],[.84,.80,.065],[.30,.30,.06],[.75,.59,.08],[.70,.70,.055],[.50,.87,.06],[.87,.63,.065],[.53,.18,.06],[.40,.18,.055]]};
    automaticDifferenceDescriptions['pakjesavond']=['Sinterklaas knipoogt.','In de krul van de staf staat een ster.','De kaars op de schouw is verdwenen.','De maan aan het raam is een zon geworden.','De wortel is uit de schoen verdwenen.','De teddybeer draagt een strik.','Het ronde pakje heeft minder stippen.','Op de laars staat een hartje.','De zak heeft nog maar één kwastje.','Op een vlaggetje staat een extra kruis.'];
    automaticDifferenceDescriptions['stoomboot']=['De vensters van de vuurtoren zijn verdwenen.','De meeuw linksboven draagt een matrozenpet.','De schoorsteen heeft een extra streep.','De vlag links op het schip heeft een andere punt.','Een patrijspoort heeft geen binnenring meer.','Het anker mist een zijpunt.','Sinterklaas knipoogt.','Het pakje van de helper heeft minder lint.','De pompon op de muts is een ster geworden.','Op de rechtervlag staat een kruis.'];
    automaticDifferencePoints['pakjesavond']=[[.46,.225,.04],[.586,.10,.045],[.075,.16,.055],[.835,.11,.05],[.088,.365,.05],[.075,.725,.045],[.92,.85,.06],[.242,.87,.045],[.043,.835,.04],[.185,.12,.04]];
    automaticDifferencePoints['stoomboot']=[[.083,.29,.055],[.22,.095,.055],[.526,.18,.055],[.385,.25,.06],[.477,.595,.04],[.84,.62,.055],[.69,.255,.04],[.815,.46,.05],[.052,.55,.04],[.94,.28,.05]];
    ...
    automaticDifferencePoints['kerstkamer']=[[.18,.075,.04],[.214,.25,.04],[.49,.52,.055],[.73,.67,.045],[.49,.235,.045],[.625,.30,.045],[.145,.86,.045],[.582,.505,.045],[.785,.18,.07],[.91,.78,.045]];
    automaticDifferencePoints['wintermarkt']=[[.838,.783,.05],[.21,.84,.05],[.426,.39,.04],[.485,.36,.04],[.68,.13,.04],[.855,.36,.045],[.675,.76,.055],[.047,.88,.04],[.17,.575,.045],[.87,.61,.055]];
    ...
    automaticDifferencePoints['paaseieren-zoeken']=[[.45,.54,.04],[.82,.27,.04],[.305,.61,.045],[.84,.60,.05],[.475,.78,.055],[.845,.79,.06],[.215,.88,.05],[.735,.89,.055],[.37,.18,.08],[.60,.75,.04]];
    automaticDifferencePoints['paastuin']=[[.48,.36,.045],[.53,.55,.05],[.875,.26,.05],[.235,.42,.055],[.445,.80,.05],[.255,.13,.045],[.18,.75,.04],[.82,.79,.05],[.065,.80,.045],[.115,.17,.04]];
    ...
    automaticDifferencePoints['lentetuin']=[[.43,.47,.055],[.69,.69,.065],[.63,.60,.065],[.895,.23,.055],[.475,.13,.065],[.275,.17,.07],[.33,.29,.055],[.465,.87,.065],[.23,.45,.075],[.91,.75,.075]];
    automaticDifferencePoints['boerderij-in-de-lente']=[[.605,.40,.045],[.84,.51,.055],[.84,.70,.045],[.495,.43,.05],[.76,.18,.05],[.59,.10,.045],[.20,.60,.05],[.35,.67,.055],[.515,.79,.04],[.055,.81,.045]];
    ...
    automaticDifferenceDescriptions['verkleedfeest']=['De schedel op de piratenhoed is een hart geworden.','Op het ruimtepak staat een grote ster.','De clown heeft een groot hart op zijn pak.','De feesthoed van de clown is verdwenen.','De dinosaurus heeft minder tanden.','Het midden van de kattenstrik is een hart geworden.','De trommel heeft cirkels in plaats van driehoeken.','De rechter maraca heeft stippen in plaats van zigzaglijnen.','De stervormige toverstaf is een hart geworden.','Op het cakeje staat een groot hart.'];
    automaticDifferencePoints['carnavalsstoet']=[[.73,.55,.06],[.43,.31,.065],[.24,.42,.06],[.28,.67,.055],[.684,.516,.065],[.529,.166,.065],[.536,.617,.055],[.477,.645,.055],[.92,.59,.075],[.216,.11,.065]];
    automaticDifferencePoints['verkleedfeest']=[[.13,.27,.07],[.36,.40,.065],[.61,.43,.07],[.61,.23,.085],[.22,.71,.075],[.40,.75,.055],[.55,.86,.075],[.88,.70,.065],[.76,.37,.07],[.96,.30,.055]];
    ...
    automaticDifferencePoints['sneeuwpret']=[[.434,.477,.045],[.43,.706,.06],[.16,.70,.07],[.80,.78,.055],[.62,.10,.065],[.47,.41,.06],[.18,.53,.055],[.70,.64,.05],[.64,.45,.055],[.47,.57,.07]];
    automaticDifferencePoints['winterdorp']=[[.275,.065,.06],[.285,.33,.055],[.16,.14,.055],[.06,.47,.06],[.315,.73,.055],[.31,.58,.055],[.55,.73,.055],[.82,.71,.045],[.80,.87,.055],[.68,.91,.07]];
    ...
    automaticDifferencePoints['stranddag']=[[.305,.51,.06],[.33,.66,.05],[.15,.50,.07],[.105,.22,.06],[.325,.16,.065],[.455,.30,.055],[.645,.40,.045],[.825,.53,.055],[.715,.84,.075],[.58,.91,.06]];
    automaticDifferencePoints['kamperen']=[[.32,.51,.05],[.33,.54,.05],[.245,.40,.05],[.745,.52,.05],[.61,.59,.055],[.55,.53,.05],[.39,.26,.06],[.275,.83,.06],[.39,.74,.06],[.89,.52,.06]];
    ...
    automaticDifferencePoints['vriendschapsfeest']=[[.155,.375,.055],[.42,.50,.07],[.325,.49,.055],[.59,.14,.05],[.095,.18,.08],[.61,.52,.07],[.71,.48,.06],[.33,.78,.05],[.74,.87,.075],[.52,.78,.085]];
    automaticDifferencePoints['hartjestuin']=[[.215,.54,.055],[.70,.52,.06],[.66,.64,.055],[.49,.27,.065],[.13,.10,.08],[.31,.08,.065],[.87,.35,.075],[.16,.84,.085],[.72,.89,.065],[.91,.70,.06]];
    ...
    automaticDifferencePoints['dierenpark']=[[.54,.055,.045],[.34,.34,.045],[.37,.24,.07],[.78,.18,.08],[.82,.50,.055],[.57,.62,.045],[.20,.65,.05],[.37,.74,.06],[.71,.69,.06],[.83,.78,.075]];
    automaticDifferencePoints['boerderijdieren']=[[.40,.73,.05],[.34,.31,.045],[.53,.20,.07],[.52,.51,.045],[.09,.66,.07],[.09,.12,.08],[.36,.16,.06],[.72,.72,.065],[.37,.87,.065],[.88,.40,.07]];
    Object.assign(automaticDifferenceDescriptions, {
      ...
      'drukkerij':['Raamlat weg.','Boeken minder.','EÃ©n inktpot.','Ander zetwerktuig.','Lettervak leeg.','Inktrol is veerpen.','Man houdt boek.','Pershendel weg.','Bedrukte bladen.','Boek minder.'],
    ...
    function startAction(e) {
        ...
        if(currentTool==='tekst'){const leftText=textOriginal.value.trim(),rightText=textDifferent.value.trim();if(!leftText&&!rightText){statusText.textContent='Vul eerst tekst voor links of rechts in.';textDifferent.focus();return;}const size=Math.max(10,Math.min(60,Number(textSize.value)||24));[ctxOrigineel,ctxVerschillen].forEach(ctx=>{ctx.save();ctx.font=`bold ${size}px Arial`;ctx.fillStyle=currentColor;ctx.textBaseline='top';});if(leftText)ctxOrigineel.fillText(leftText,pos.x,pos.y);if(rightText)ctxVerschillen.fillText(rightText,pos.x,pos.y);ctxOrigineel.restore();ctxVerschillen.restore();saveState();statusText.textContent='Tekst toegevoegd. Klik opnieuw om dezelfde tekst nogmaals te plaatsen.';return;}
    ...
    function saveState() {
        ...
        undoStack.push({original:ctxOrigineel.getImageData(0,0,canvasOrigineel.width,canvasOrigineel.height),different:ctxVerschillen.getImageData(0,0,canvasVerschillen.width,canvasVerschillen.height)});
        undoBtn.disabled = undoStack.length <= 1;
    }

    function restoreState(state){if(!state)return;if(state.original&&state.different){ctxOrigineel.putImageData(state.original,0,0);ctxVerschillen.putImageData(state.different,0,0);}else{ctxVerschillen.putImageData(state,0,0);}}
    ...
    function downloadPuzzel(format,solutionMode=false) {
        ...
        if (format === 'pdf') {
            ...
            const titleBase=worksheetTitle?.value.trim()||'Zoek de verschillen',title=solutionMode?`Oplossing - ${titleBase}`:titleBase;
            ...
            doc.setTextColor(55, 55, 65); doc.setFont('helvetica', 'normal'); doc.setFontSize(10.5);
            ...
            doc.setDrawColor(185, 190, 202); doc.line(margin, 20, pageWidth - margin, 20);
            doc.setFont('helvetica', 'bold'); doc.setFontSize(20); doc.setTextColor(35, 74, 128);
            ...
            doc.setFont('helvetica', 'normal'); doc.setFontSize(11); doc.setTextColor(50, 50, 58);
            const instructionLines = doc.splitTextToSize(instruction, pageWidth - (margin * 2));
            doc.text(instructionLines, margin, instructionY);
            const imageTop = instructionY + (instructionLines.length * 5.2) + 7, imageGap = 9;
            const availableWidth = pageWidth - (margin * 2), contentHeight = pageHeight - imageTop - 17;
            ...
            const imageWidth=canvasOrigineel.width*scale,imageHeight=canvasOrigineel.height*scale;
            ...
            doc.setDrawColor(55, 92, 132); doc.setLineWidth(.45);
            doc.addImage(canvasOrigineel.toDataURL('image/png'), 'PNG', firstX, firstY, imageWidth, imageHeight); doc.rect(firstX, firstY, imageWidth, imageHeight);
            doc.addImage(exportDifferenceCanvas.toDataURL('image/png'), 'PNG', secondX, secondY, imageWidth, imageHeight); doc.rect(secondX, secondY, imageWidth, imageHeight);
            doc.setFontSize(8); doc.setTextColor(145, 145, 150);
