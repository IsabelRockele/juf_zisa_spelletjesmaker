(function () {
  ...
  function injecteerStijl(doc) {
    ...
    stijl.textContent = `
      ...
      .routine-blok.boekentas-blok { grid-column:1/-1; }
      .routine-blok.boekentas-blok ul { display:grid; grid-template-columns:repeat(3,minmax(0,1fr)); gap:14px 30px; margin:8px 0 14px; }
      .routine-blok.boekentas-blok li { position:relative; min-width:0; margin:0; padding:10px; }
      .routine-blok.boekentas-blok li:not(:nth-child(3n)):not(:last-child)::after { content:'+'; position:absolute; right:-23px; top:50%; transform:translateY(-50%); color:#6754bd; font-size:31px; font-weight:1000; }
      .routine-blok.boekentas-blok .routine-toevoegen { width:100%; }
      @media(max-width:850px){.routine-blok.boekentas-blok ul{grid-template-columns:repeat(2,minmax(0,1fr))}.routine-blok.boekentas-blok li::after{display:none}}
  ...
  function renderOchtend(doc, root, data, bewerken, wijzig, opnieuw) {
    ...
    const routineKeuzes=[['brooddoos-broodbak','Brooddoos'],['drinkbus-vaste-plek','Drinkbus'],['snack-fruit-bak','Koek en fruit'],['agendamap-tafel','Agendamap'],['brieven-afgeven','Brieven'],['huistaak-afgeven','Huistaak'],['boekentas-opbergen','Boekentas'],['stille-dagstarter','Stille dagstarter'],['lezen','Lezen']];
    const openRoutineKiezer=(item,nieuw=false)=>{let gekozen=item.icoon||'';const laag=node(doc,'div','programma-keuzelaag'),venster=node(doc,'section','programma-kiezer'),kop=node(doc,'div','programma-kiezer-kop'),sluit=node(doc,'button','','×'),gridKeuzes=node(doc,'div','programma-keuzegrid'),formulier=node(doc,'div');const titelVeld=node(doc,'input'),tekstVeld=node(doc,'input'),bewaar=node(doc,'button','rijk-toevoegen','Routine bewaren');kop.append(node(doc,'h2','',nieuw?'Nieuwe routine maken':'Routine aanpassen'),sluit);sluit.type='button';sluit.onclick=()=>laag.remove();titelVeld.placeholder='Titel, bijvoorbeeld Lezen';titelVeld.value=nieuw?'':item.titel||'';tekstVeld.placeholder='Extra tekst, bijvoorbeeld Lees stil in je boekje';tekstVeld.value=nieuw?'':item.tekst||'';[titelVeld,tekstVeld].forEach(veld=>veld.style.cssText='width:100%;margin-top:10px;padding:11px 13px;border:2px solid #d9d2ef;border-radius:11px;font:inherit;box-sizing:border-box;');const werkSelectieBij=()=>gridKeuzes.querySelectorAll('button').forEach(knop=>knop.style.outline=knop.dataset.icoon===gekozen?'4px solid #6754bd':'');routineKeuzes.forEach(([icon,label])=>{const knop=node(doc,'button','programma-keuze');knop.type='button';knop.dataset.icoon=icon;knop.append(icoon(doc,icon),node(doc,'span','',label));knop.onclick=()=>{gekozen=icon;if(!titelVeld.value.trim())titelVeld.value=label;werkSelectieBij();};gridKeuzes.appendChild(knop);});werkSelectieBij();bewaar.type='button';bewaar.style.marginTop='12px';bewaar.onclick=()=>{if(!gekozen){doc.defaultView.alert('Kies eerst een afbeelding.');return;}item.icoon=gekozen;item.titel=titelVeld.value.trim()||'Nieuwe routine';item.tekst=tekstVeld.value.trim();if(nieuw)data.items.push(item);wijzig();laag.remove();opnieuw();};formulier.append(titelVeld,tekstVeld,bewaar);venster.append(kop,gridKeuzes,formulier);laag.appendChild(venster);doc.body.appendChild(laag);titelVeld.focus();};
    data.items.forEach((item, index) => {
      ...
      const beeld=node(doc,bewerken?'button':'span','routine-icoonknop');if(bewerken){beeld.type='button';beeld.title='Afbeelding en tekst aanpassen';beeld.onclick=()=>openRoutineKiezer(item);}beeld.appendChild(icoon(doc,item.icoon));
      rij.append(node(doc,'span','stapnummer',`stap ${index+1}`),beeld,tekst);
  ...
  function renderRoutines(doc,root,data,bewerken,wijzig,opnieuw){
    ...
    const keuzes=[
      ...
      ['losse-drinkbus','Drinkbus'],['losse-brooddoos','Brooddoos'],['losse-koekendoos','Koekendoos'],['losse-fruitdoos','Fruitdoos'],['losse-agendamap','Agendamap'],['losse-huistaken','Huistaken'],['losse-pennenzak','Pennenzak'],
