// @ts-nocheck
/* ══════════════════════════════════════════════════════════════
   getalinzicht.js  —  oefening-renderers + UI-koppeling
   Preview : gi-preview.js  (getallenlijn preview)
   PDF     : gi-pdf.js      (exporteren)
   ══════════════════════════════════════════════════════════════ */
(function(){
  ...
  // Hulpfunctie voor getallenlijn blanks-modus (ook gebruikt door gi-preview.js intern)
  function wantLabel(val, o) {
...
// (vul de cellen zoals u al deed…)
  ...
  card.classList.add('row-delete-wrap');
...
if (field) {
  ...
  _registerAddFn(key, () => _voegHvpItemToe(key));
...
function addHvIconsExercises(){
  ...
  _registerAddFn(key, () => _voegHvIconsItemToe(key));
...
function addGetalbeelden1000(){
  ...
  _registerAddFn(key, () => _voegGb1000ItemToe(key));
  ...
  for(let i=0;i<count;i++){
    ...
    if (maxVal <= 100) {
      visual.querySelectorAll('.gb1000-hvwrap').forEach(el => {
        el.classList.add('gb1000-hvwrap-groot');
      });
      visual.classList.add('gb1000-visual-enkel');
    }
    card.appendChild(visual);

    // invulschema: T/E tabel + numBox
  ...
  <tr>
    ...
    if (maxVal <= 100) {
      // Tot 100: visual boven, tabel + numBox eronder gecentreerd
      // Gebruik block layout op card zodat visual en task echt onder elkaar staan
      card.style.display = 'flex';
      card.style.flexDirection = 'column';
      card.style.alignItems = 'center';
      card.style.gap = '8px';
      const task = document.createElement('div');
      task.className = 'gb1000-task-onder';
      task.append(table, numBox);
      card.appendChild(task);
    } else {
      // Tot 1000: TE-tabel uitbreiden met H-kolom
      table.innerHTML = `
  <tr>
    <td class="te-label h" style="background:#42a5f5;color:#fff">H</td>
    <td class="te-label t">T</td>
    <td class="te-label e">E</td>
  </tr>
  <tr>
...
(function(){
  ...
  // Map van titleKey → addFunctie (voor de "+ oefening"-knop)
  const _addFnForKey = {};
  ...
  function ensureTitleOnce(container, key, text){
    if (!addedTitles.has(key)) {
      ...
      // Blauwe titelbalk met lijn
      const inner = document.createElement('div');
      inner.className = 'title-row-inner';

      // Opdrachtzin als titel in blauwe balk
      ...
      addBtn.type = 'button';
      addBtn.className = 'title-add-btn';
      addBtn.innerHTML = '+ oefening';
      addBtn.addEventListener('click', () => {
        const fn = _addFnForKey[key];
        if (fn) fn();
      });
      ...
      delBtn.type = 'button';
      delBtn.className = 'title-delete-btn';
      delBtn.innerHTML = '\u00D7';
...
function placeAfterLastOfKey(el, key){
  ...
  const createDeleteButton = (target) => {
    ...
    btn.addEventListener('click', () => {
      ...
      if (!remainingBlock) {
        const titleRow = document.querySelector(`.exercise-title[data-title-key="${titleKey}"]`)?.closest('.title-row');
...
function createRowDeleteButton(target){
  ...
  /* ── Getallenlijn naar sheet ───────────────────────────── */
  function addExerciseToSheet(){
    const o = window.GI_Preview ? window.GI_Preview.getRulerOpts() : {}; if(o.end<=o.start){alert('Tot en met moet groter zijn dan Van.');return;}
    ...
    if (titleKey) { ensureTitleOnce(sheet, titleKey, titleText); _registerAddFn(titleKey, addExerciseToSheet); }
...
requestAnimationFrame(() => {
  ...
  if (o.mode==='drag') {
    // Clip de SVG tot net onder de lijn (geen lege ruimte boven labels)
    // Gebruik viewBox om alleen het relevante deel te tonen
    const svgHeight = svg.getBoundingClientRect().height || 240;
    const svgWidth2 = svg.getBoundingClientRect().width || 700;
    const vbH = Math.round(baseY + 22);  // lijn + wat ruimte onder
    const vbY = 0;
    svg.setAttribute('viewBox', `0 ${vbY} ${svgWidth2} ${vbH}`);
    svg.style.height = Math.round(svgHeight * vbH / 240) + 'px';
    if ((o.end - o.start) > 100) {
      const warn = document.createElement('div');
      warn.className = 'ruler-warning';
      warn.textContent = '⚠️ Bereik groter dan 100: de getallenlijn past mogelijk niet volledig in de PDF. Verklein het bereik (Van / Tot).';
      ex.appendChild(warn);
    }
    ...
    // Kaarten in een flex-container die automatisch centreert
    // Dit werkt correct op elke breedte (scherm én PDF-export)
    const kaartRij = document.createElement('div');
    kaartRij.className = 'drag-cards-rij';
    kaartRij.style.cssText = 'display:flex;justify-content:center;gap:16px;margin-top:12px;flex-wrap:wrap;';
    values.forEach(val=>{
      // Wrapper met bolletje + kaartje onder elkaar
      const wrap = document.createElement('div');
      wrap.style.cssText = 'display:flex;flex-direction:column;align-items:center;gap:4px;';

      // Bolletje (SVG-stip bovenaan)
      // Klein bolletje als gewone div
      const dot = document.createElement('div');
      dot.style.cssText = 'width:8px;height:8px;border-radius:50%;background:#111;flex-shrink:0;';
      ...
      kaartRij.appendChild(wrap);
    });
    ex.appendChild(kaartRij);
  }

  if (o.mode==='blanks'){
    // Kies punten verspreid over de lijn via zones
    const svgBreedte = svg.getBoundingClientRect().width || 700;
    const nZones = o.blankCount;
    const zoneB  = (svgBreedte - PAD_L - PAD_R) / nZones;
    const picked = [];
    for (let z = 0; z < nZones; z++) {
      const xMin = PAD_L + z * zoneB + 4;
      const xMax = PAD_L + (z + 1) * zoneB - 4;
      const kand = ticks.filter(t => !wantLabel(t.v, o) && t.x >= xMin && t.x < xMax);
      if (kand.length) picked.push(kand[Math.floor(Math.random() * kand.length)]);
    }
    picked.sort((a, b) => a.x - b.x);

    const bw = 52, bh = 36;
    const pijlL = 40;  // pijllengte boven én onder gelijk
    const svgRect = svg.getBoundingClientRect();
    const exRect  = ex.getBoundingClientRect();

    // Hokje BOVEN: vast bovenaan SVG (y=4), ver boven de labels
    // Labels staan op baseY-20..baseY (28px hoog) → hokje eindigt op 40px, label start op baseY-48
    const yBovenTop = 4;
    // Hokje ONDER: baseY + pijlL
    const yOnderTop = baseY + pijlL;

    let above = true;
    picked.forEach(t => {
      const xC = t.x;
      const boxTop = above ? yBovenTop : yOnderTop;

      // Pijl eerst: eindigt EXACT op de hokje-rand
      // arrowhead-marker heeft refY=4, markerHeight=8 → pijlpunt steekt 4px voorbij y2
      // Compenseer: y2 = hokjerand + 4 (voor boven) zodat punt op rand valt
      const pijlY1 = above ? baseY - 2 : baseY + 2;
      const pijlY2 = above ? boxTop + bh + 4 : boxTop - 4;
      const conn = line(g, xC, pijlY1, xC, pijlY2, '#1e88e5', '1.8');
      conn.setAttribute('marker-end', 'url(#arrowhead)');

      // Rect DAARNA: staat op boxTop zodat de pijlsteel zichtbaar is maar punt achter rect
      const rect = document.createElementNS(NS, 'rect');
      rect.setAttribute('x', xC - bw/2);
      rect.setAttribute('y', boxTop);
      rect.setAttribute('width', bw);
      rect.setAttribute('height', bh);
      rect.setAttribute('rx', 9);
      rect.setAttribute('fill', '#fff');
      rect.setAttribute('stroke', '#6b879a');
      rect.setAttribute('stroke-width', '2');
      g.appendChild(rect);

      // HTML input (voor invullen op scherm)
      const pxLeft = (svgRect.left - exRect.left) + xC - bw/2;
      const pxTop  = (svgRect.top  - exRect.top)  + boxTop;
      const inp = document.createElement('input');
      inp.type = 'text';
      inp.className = 'print-overlay-input';
      inp.style.cssText = `position:absolute;left:${pxLeft}px;top:${pxTop}px;width:${bw}px;height:${bh}px;font-size:16px;font-weight:700;text-align:center;background:transparent;border:0;outline:none;z-index:3;box-sizing:border-box;`;
      ex.appendChild(inp);
...
t.setAttribute('dominant-baseline','text-before-edge'); // stabieler in PDF
  ...
  function addJumpExercise() {
    ...
    _registerAddFn(key, () => _voegJumpItemToe(key));
...
function isCleanTen(n){ return n % 10 === 0; }
  ...
  function makeHTE(max) {
    // Genereer een HTE-notatie voor getallen tot max (min 100)
    const effectief = Math.max(100, Math.min(max, 999));
    const patterns = ['H', 'T', 'E', 'HT', 'HE', 'TE', 'HTE'];
    const p = patterns[Math.floor(Math.random() * patterns.length)];
    let h = 0, t = 0, e = 0, parts = [];

    if (p === 'H') {
      h = 1 + Math.floor(Math.random() * Math.floor(effectief / 100));
      parts.push(`${h}H`);
      return { text: parts.join(' '), value: h * 100 };
    }
    if (p === 'T') {
      t = 1 + Math.floor(Math.random() * 9);
      parts.push(`${t}T`);
      return { text: parts.join(' '), value: t * 10 };
    }
    if (p === 'E') {
      e = 1 + Math.floor(Math.random() * 9);
      parts.push(`${e}E`);
      return { text: parts.join(' '), value: e };
    }
    // gecombineerde vormen
    const maxH = Math.max(1, Math.floor(effectief / 100));
    if (p.includes('H')) h = 1 + Math.floor(Math.random() * maxH);
    if (p.includes('T')) t = Math.floor(Math.random() * 10);
    if (p.includes('E')) e = Math.floor(Math.random() * 10);
    const value = h * 100 + t * 10 + e;
    const sep = Math.random() < 0.5 ? ' en ' : ' ';
    if (p === 'HT')  parts = [`${h}H`, `${t}T`];
    if (p === 'HE')  parts = [`${h}H`, `${e}E`];
    if (p === 'TE')  parts = [`${t}T`, `${e}E`];
    if (p === 'HTE') parts = [`${h}H`, `${t}T`, `${e}E`];
    return { text: parts.join(sep), value };
  }
  ...
  function makeSideHTEorNumber(max) {
    if (Math.random() < 0.5) {
      ...
      const v = Math.max(100, rnd(Math.min(max, 999)));
      return { isHTE: false, text: String(v), value: v };
    }
  }


  /* ── Voeg 1 mix-item toe aan bestaand blok ─────────────── */
  function _voegMixItemToe(key, type, max) {
    // Zoek het laatste blok van dit type
    ...
    const grid = blok.querySelector('.mixed-grid');
    if (!grid) { addMixedExercises(); return; }

    // Maak één nieuw mix-item (zelfde logica als in de loop van addMixedExercises)
    const n = parseInt($('#mixedMax')?.value, 10) || max || 100;
    ...
    function makeNum(v){const d=document.createElement('div');d.className='mix-num';d.textContent=v;return d;}
    function makeLabel(t){const s=document.createElement('span');s.className='mix-label';s.textContent=t;return s;}

    // Genereer een willekeurige nieuwe oefening (vermijd herhaling van de laatste)
    let tries = 0;
    const bestaandeGetallen = new Set(Array.from(blok.querySelectorAll('.mix-num')).map(el=>parseInt(el.textContent)));
    let x;
    do { x = rnd(n); tries++; } while (bestaandeGetallen.has(x) && tries < 20);

    if (type==='neighbors'||type==='neighborTens'||type==='neighborHundreds'){
      item.append(box(), makeNum(x), box());
    } else if (type==='compare'){
      const b=rnd(n); item.append(makeNum(x), box('small'), makeNum(b));
    } else if (type==='composeTE'){
      const te=makeTE(n); const eq=document.createElement('span'); eq.className='mix-eq'; eq.textContent='=';
      item.append(makeLabel(te.text), eq, box());
    } else if (type==='compareTE'){
      const l=makeSideTEorNumber(n), r=makeSideTEorNumber(n);
      item.append(l.isTE?makeLabel(l.text):makeNum(l.value), box('small'), r.isTE?makeLabel(r.text):makeNum(r.value));
    } else if (type==='composeHTE'){
      const hte=makeHTE(n); const eq=document.createElement('span'); eq.className='mix-eq'; eq.textContent='=';
      item.append(makeLabel(hte.text), eq, box());
    ...
    // Vul eerst mixed-first aan als die nog niet vol is
    const first = blok.querySelector('.mixed-first');
    const isHTE  = grid.classList.contains('hte-2col');
    const isFill = grid.classList.contains('fillnext-grid');
    const cols   = isHTE ? 2 : (isFill ? 2 : 3);
    const firstCount = first ? first.querySelectorAll('.mix-item').length : cols;

    if (first && firstCount < cols) {
  ...
  nextHundred: 'mixed_next_hundred'
    ...
    // + oefening: voegt 1 item toe aan het bestaande grid, niet een heel nieuw blok
    _registerAddFn(key, (type==='nextTen'||type==='nextHundred') ? () => _voegFillNextItemToe(key, type) : () => _voegMixItemToe(key, type, max));
...
let under100Used = false;
  ...
  vragen.className = 'fillnext-vragen';
  vragen.innerHTML = `
    <div class="fillnext-num-groot">${x}</div>
...
placeAfterLastOfKey(block, key);
  ...
  function addSequenceExercise(){
    ...
    _registerAddFn(key, () => _voegSequenceItemToe(key));
...
function addHonderdveldExercise() {
  ...
  for (let i = 0; i < count; i++) {
    // Blok + verwijderknop
    const block = document.createElement('div');
    block.className = 'honderdveld-exercise-block row-delete-wrap';
    block.dataset.titleKey = key;
    block.appendChild(createRowDeleteButton(block));
    // Container
    ...
    // Layout: grid BOVEN, task ONDER (verticaal)
    ...
    if (type === 'veld-naar-getal') {
      ...
      // TE-tabel + numBox onder het grid
      const task = document.createElement('div');
      task.className = 'honderdveld-task-onder';
      const table = document.createElement('table');
      table.className = 'honderdveld-te-table';
      table.innerHTML = `
  <tr>
  ...
  <tr>
    ...
    // Altijd 3 per rij (via honderdveld-row met max 3)
    const perRij = 3;
  ...
  function addMabExercise(){
    ...
    const block=document.createElement('div'); block.className='mab-exercise-block'; block.dataset.titleKey=key;
...
// verwijdering via blok-niveau delete-knop
      ...
      // Tot 100: 3 per rij. Tot 1000: 2 per rij (MAB-blokken zijn breed)
      const perRij = max <= 100 ? 3 : 2;
      const mabHead = document.createElement('div');
      mabHead.className = 'mab-first keep-with-title';
      mabHead.style.display = 'grid';
      mabHead.style.gridTemplateColumns = `repeat(${perRij}, minmax(0, 1fr))`;
      mabHead.style.gap = getComputedStyle(grid).gap || '16px';

      // verplaats de eerste N kinderen naar de head
      for (let i = 0; i < perRij && grid.firstChild; i++) {
        mabHead.appendChild(grid.firstChild);
      }

      // grid (rest): ook perRij kolommen
      grid.style.gridTemplateColumns = `repeat(${perRij}, minmax(0, 1fr))`;
...
function alignPlaceValueHTE(item){
  ...
  // ★ KORTE pijlen
  const LAND_E = yStartE + 50;   // E-rij
  const LAND_T = yStartT + 110;  // T-rij
  const LAND_H = yStartH + 170;  // H-rij
...
svg.style.height = h + 'px';
 ...
 function addPlaceValueExercise() {
    ...
    if (mode === 'connect' || mode === 'color') {
      ...
      _registerAddFn(key, () => _voegPvConnectItemToe(key, mode));
...
addCells(botCards, bandBot, 'bot');   // punt bovenaan
  ...
  grid.appendChild(row);
    ...
    if (range === '1000') {
      ...
      _registerAddFn(key, () => _voegPvHteItemToe(key));
...
rowT.innerHTML = `<input type="text" class="pv3-small"> <span class="pv3-label">T =</span> <input type="text" class="pv3-box pv3-box-wide">`;

const rowH = document.createElement('div'); rowH.className = 'pv3-row';
// E weggelaten om compacter te zijn
rowH.innerHTML = `<input type="text" class="pv3-small"> <span class="pv3-label">H =</span> <input type="text" class="pv3-box"> <span class="pv3-label">T =</span> <input type="text" class="pv3-box pv3-box-wide">`;
...
_registerAddFn(key, () => _voegPvTeItemToe(key));
const block = document.createElement('div');
block.className = 'placevalue-exercise-block';
block.dataset.titleKey = key;
block.appendChild(createDeleteButton(block));
...
if (range === '20') {
  ...
  const item = document.createElement('div');
  item.className = 'pv-item row-delete-wrap';
  item.appendChild(createRowDeleteButton(item));
...
placeAfterLastOfKey(block, key);
  ...
  function _rekentaalOpgave(vorm, afstand, max) {
    ...
    if (vorm === 'beforeAfter') { basis = Math.floor(Math.random()*(max-afstand+1)); antwoord=basis+afstand; tekst=`${basis} komt ${stapTekst} voor`; }
    else if (vorm === 'blankBefore') { basis=afstand+Math.floor(Math.random()*(max-afstand+1)); antwoord=basis-afstand; tekst=`komt ${stapTekst} voor ${basis}`; }
    else if (vorm === 'afterBlank') { basis=afstand+Math.floor(Math.random()*(max-afstand+1)); antwoord=basis-afstand; tekst=`${basis} komt ${stapTekst} na`; }
    else { basis=Math.floor(Math.random()*(max-afstand+1)); antwoord=basis+afstand; tekst=`komt ${stapTekst} na ${basis}`; }
    return {vorm,tekst,antwoord};
  }
  function _maakRekentaalItem(vorm, afstand, max) {
    const opgave=_rekentaalOpgave(vorm,afstand,max), item=document.createElement('div'), input=document.createElement('input');
    item.className='rekentaal-item row-delete-wrap'; item.dataset.antwoord=String(opgave.antwoord); item.appendChild(createRowDeleteButton(item));
  ...
  function _voegRekentaalItemToe(key,vormen,afstandKeuze,max){
    const blokken=Array.from(sheet.querySelectorAll(`.rekentaal-exercise-block[data-title-key="${key}"]`)); if(!blokken.length){addRekentaalExercises();return;}
    const grid=blokken.at(-1).querySelector('.rekentaal-grid'),vorm=vormen[Math.floor(Math.random()*vormen.length)],afstand=afstandKeuze==='mix'?(Math.random()<.5?1:2):Number(afstandKeuze),item=_maakRekentaalItem(vorm,afstand,max);
    if(grid.children.length<2)item.classList.add('rekentaal-eerste-rij'); grid.appendChild(item); requestAnimationFrame(()=>window.GI_Toets?.updateScoreVakken());
  }
  function addRekentaalExercises(){
    const max=parseInt($('#rekentaalMax')?.value,10)||20,count=clamp(parseInt($('#rekentaalCount')?.value,10)||8,2,30),afstandKeuze=document.querySelector('input[name="rekentaalAfstand"]:checked')?.value||'1',vormen=$$('.rekentaal-vorm:checked').map(el=>el.value);
    if(!vormen.length){alert('Kies minstens één zinsvorm.');return;}
    const key=`rekentaal_${max}_${afstandKeuze}_${vormen.join('_')}`,titel=afstandKeuze==='2'?'Vul het getal 2 plaatsen voor of na in.':afstandKeuze==='mix'?'Vul het juiste getal voor of na in.':'Vul het getal net voor of net na in.';
    ensureTitleOnce(sheet,key,titel); _registerAddFn(key,()=>_voegRekentaalItemToe(key,vormen,afstandKeuze,max));
    const block=document.createElement('div'),grid=document.createElement('div'); block.className='rekentaal-exercise-block'; block.dataset.titleKey=key; block.appendChild(createDeleteButton(block)); grid.className='rekentaal-grid';
    for(let i=0;i<count;i++){const vorm=vormen[i%vormen.length],afstand=afstandKeuze==='mix'?(i%2)+1:Number(afstandKeuze);grid.appendChild(_maakRekentaalItem(vorm,afstand,max));}
    Array.from(grid.children).slice(0,2).forEach(item=>item.classList.add('rekentaal-eerste-rij')); block.appendChild(grid); sheet.appendChild(block); requestAnimationFrame(()=>window.GI_Toets?.scanEnVoeg());
  }
  ...
  function renderSheetHeader(){ if (sheet.querySelector('.sheetHeader')) return;
    ...
    titleEl.textContent='Extra oefenen op getalinzicht';
    titleEl.style.cssText='display:inline-block;font-size:26px;font-weight:700;color:#1A3A5C;'
      +'min-width:200px;outline:none;border-bottom:2px dashed transparent;cursor:text;'
      +'transition:border-color .2s;padding:2px 6px;border-radius:4px;';
    titleEl.title='Klik om de titel aan te passen';
    titleEl.addEventListener('focus',()=>{ titleEl.style.borderBottomColor='#4A90D9'; titleEl.style.background='#f0f8ff'; });
    titleEl.addEventListener('blur', ()=>{ titleEl.style.borderBottomColor='transparent'; titleEl.style.background=''; });
...
bindThrottled($('#btnAddPlaceValue'),   addPlaceValueExercise);
  ...
  document.addEventListener('DOMContentLoaded', () => {
    renderSheetHeader();
    if (window.GI_Preview) {
      GI_Preview.renderPreview();
      GI_Preview.updateDragSuggestions();
    }
  });
