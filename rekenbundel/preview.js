...
const Preview = (() => {
  ...
  function _oefeningHTML(blok, oef, idx) {
    ...
    const isTot20AftrekkenMetLijnen = heeftLijnen && oefBewerking === 'aftrekken' && Number(blok.niveau) === 20;
    ...
    return `
      <div class="oefening-item oefening-hulp${isVoorbeeld ? ' tussenstappen-voorbeeld' : ''}${isTot20AftrekkenMetLijnen ? ' tot20-aftrekken-lijnen' : ''}${isAftrektal ? ' aftrektal-hulp' : ''}${isAftrektalGroot ? ' aftrektal-hulp-groot' : ''}${splGroot ? ' niveau-10000' : ''}">
        <div class="hulp-som-rij">
          <span class="oef-tekst">${somHTML}</span>
          ${isTot20AftrekkenMetLijnen
            ? `<span class="tussenstappen-eerste-lijn${isVoorbeeld ? ' tussenstappen-start-ingevuld' : ''}" data-antwoord="${isTwintigMinTE ? `(20 − 10) − ${voorbeeldEenheid}` : (oef.antwoord ?? '')}">${isVoorbeeld ? (isTwintigMinTE ? `(20 − 10) − ${voorbeeldEenheid}` : esc(oef.antwoord ?? '')) : ''}</span>`
 ...
 function _getallenlijnHTML(blokId, oef, idx) {
  ...
  function _maakRekenrelatiesElement(blok) {
    const soort=blok.config?.soort||'familie';
    const kaarten=blok.oefeningen.map((o,i)=>{
      const del=`<button class="btn-del-oef" onclick="App.verwijderOefening('${blok.id}',${i})" title="Verwijder">✕</button>`;
      if(soort==='familie') return `<div class="oefening-item rr-familie"><div class="rr-getallen">${o.getallen.map(n=>`<span>${n}</span>`).join('')}</div><div class="rr-familie-lijnen">${o.oplossingen.map(s=>`<div class="rr-schrijfregel" data-antwoord="${esc(s)}"></div>`).join('')}</div>${del}</div>`;
      if(soort==='kader'){
        const teken=o.bewerking==='aftrekken'?'−':'+'; const leeg=new Set(o.legeSleutels||[]);
        const vak=k=>`<span class="rr-kader-getal rr-k-${k} ${leeg.has(k)?'rr-kader-vak':''}" ${leeg.has(k)?`data-antwoord="${o.cellen[k]}"`:''}>${leeg.has(k)?'':o.cellen[k]}</span>`;
        return `<div class="oefening-item rr-kader"><div class="rr-kader-bank">${o.bank.map(n=>`<span>${n}</span>`).join('')}</div><div class="rr-kader-frame">${vak('tl')}<b class="rr-k-op-boven">${teken}</b>${vak('boven')}<b class="rr-k-eq-boven">=</b>${vak('tr')}<b class="rr-k-op-links">${teken}</b>${vak('links')}<b class="rr-k-eq-links">=</b>${vak('bl')}<b class="rr-k-op-rechts">${teken}</b>${vak('rechts')}<b class="rr-k-eq-rechts">=</b>${vak('br')}<b class="rr-k-op-onder">${teken}</b>${vak('onder')}<b class="rr-k-eq-onder">=</b></div>${del}</div>`;
      }
      const teken=o.bewerking==='aftrekken'?'−':'+';
      return `<div class="oefening-item rr-rooster"><table><thead><tr><th class="rr-hoek"><span>${teken}</span></th>${o.kolommen.map(n=>`<th>${n}</th>`).join('')}</tr></thead><tbody>${o.rijen.map((r,ri)=>`<tr><th>${r}</th>${o.waarden[ri].map(v=>`<td><span data-antwoord="${v}"></span></td>`).join('')}</tr>`).join('')}</tbody></table>${del}</div>`;
    }).join('');
    const div=document.createElement('div');div.className='preview-blok';div.dataset.id=blok.id;
    div.innerHTML=`<div class="preview-blok-header"><span class="blok-type-badge">🔗 ${soort==='familie'?'Getallenfamilie':soort==='kader'?'Rekenkader':'Rekenrooster'}</span><span class="blok-niveau">Tot ${blok.niveau} · ${{zonder:'zonder brug',met:'met brug',beide:'beide'}[blok.brug]||''}</span><div class="spacer"></div><button class="btn-blok-actie verwijder" onclick="App.verwijderBlok('${blok.id}')">✕</button></div><div class="preview-blok-body"><div class="opdrachtzin-wrapper">${esc(blok.opdrachtzin)}</div><div class="rr-grid rr-${soort}">${kaarten}</div></div>`;
    return div;
  }
