/* ══════════════════════════════════════════════════════════════
   modules/aftrekken-tot10000.js
   Aftrekken tot 10.000 — enkel ZONDER BRUG

   Types (parallel aan optellen-tot10000):
   D-D, DH-D, DH-H, DH-DH,
   DHT-D, DHT-H, DHT-T, DHT-DH, DHT-DHT,
   DHTE-D, DHTE-H, DHTE-T, DHTE-HT, DHTE-DHT, DHTE-DHTE, Gemengd

   Zonder brug = geen leen in geen enkele kolom:
     e1>=e2  EN  t1>=t2  EN  h1>=h2  EN  d1>=d2
     EN verschil > 0
   ══════════════════════════════════════════════════════════════ */
...
const AftrekkenTot10000 = (() => {
  ...
  function maakDH()   { return rand(1,9)*1000 + rand(1,9)*100; }
  function maakDHT()  { return rand(1,9)*1000 + rand(1,9)*100 + rand(1,9)*10; }
  function maakDHTE() { return rand(1,9)*1000 + rand(1,9)*100 + rand(1,9)*10 + rand(1,9); }
