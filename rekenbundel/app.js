...
const App = (() => {
  ...
  function toggleHulpmiddel(label, waarde) {
    ...
    if (waarde === 'schrijflijnen') {
      ...
      const isTussenstappenTot20 = actieveBewerking === 'aftrekken' &&
        parseInt(document.querySelector('[name="niveau"]:checked')?.value || 20) === 20;
...
function _getSplitsConfig() {
  ...
  // Veilige, alleen-lezen-koppeling voor andere werkbladmakers.
  // De volledige configuratie blijft in deze tool gebeuren; de aanroeper
  // ontvangt enkel een kopie van het laatst gemaakte oefenblok.
  function getLaatsteBlok() {
    const blok = bundelData[bundelData.length - 1];
    return blok ? JSON.parse(JSON.stringify(blok)) : null;
  }
...
window.RekenbundelShared = { getLaatsteBlok: App.getLaatsteBlok };
