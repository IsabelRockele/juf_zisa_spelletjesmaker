...
const PdfEngine = (() => {
  ...
  function _tekenKleinSplitshuis(oef, ox, oy) {
    ...
    // Buitendriehoek (donkerblauw)
    doc.setFillColor(74, 144, 217);
    doc.setDrawColor(74, 144, 217);
    doc.setLineWidth(0);
    doc.triangle(midX, dakTop, ox, dakBodem, ox + B, dakBodem, 'F');

    // Binnendriehoek (lichtblauw achtergrond) — iets kleiner
    const inset = 2.5;
    doc.setFillColor(234, 244, 251);
    doc.triangle(midX, dakTop + inset * 1.4, ox + inset, dakBodem - 0.5, ox + B - inset, dakBodem - 0.5, 'F');

    // Inhoud dak: getal of invulvakje
    const dakMidY = dakTop + SH_DAK_H * 0.62;
    if (oef.totaal !== null) {
      doc.setFont('helvetica', 'bold');
      doc.setFontSize(11);
      doc.setTextColor(26, 58, 92);
      ...
      doc.setDrawColor(26, 58, 92);
      doc.setLineWidth(0.5);
      doc.roundedRect(midX - vW / 2, dakMidY - vH + 1, vW, vH, 1, 1, 'FD');
    }
    ...
    doc.rect(ox, muurY, B, SH_MUUR_H, 'FD');

    // Scheidingswand midden
    doc.setDrawColor(74, 144, 217);
    doc.setLineWidth(1.0);
    doc.line(midX, muurY, midX, muurY + SH_MUUR_H);

    // Inhoud kamer links
    const kamMidY = muurY + SH_MUUR_H / 2 + 1.5;
    const kamLinksMid = ox + B / 4;
    const kamRechtsMid = ox + 3 * B / 4;
    const vW = 11, vH = 7;
    ...
    if (oef.links !== null) {
      ...
      doc.setFontSize(11);
      doc.setTextColor(26, 58, 92);
      doc.text(String(oef.links), kamLinksMid, kamMidY, { align: 'center' });
    } else {
      doc.setFillColor(255, 255, 255);
      doc.setDrawColor(26, 58, 92);
      doc.setLineWidth(0.5);
      doc.roundedRect(kamLinksMid - vW / 2, kamMidY - vH + 1, vW, vH, 1, 1, 'FD');
    }
    ...
    if (oef.rechts !== null) {
      ...
      doc.setFontSize(11);
      doc.setTextColor(26, 58, 92);
      doc.text(String(oef.rechts), kamRechtsMid, kamMidY, { align: 'center' });
    } else {
      doc.setFillColor(255, 255, 255);
      doc.setDrawColor(26, 58, 92);
      doc.setLineWidth(0.5);
      doc.roundedRect(kamRechtsMid - vW / 2, kamMidY - vH + 1, vW, vH, 1, 1, 'FD');
    }
  }
