document.addEventListener("DOMContentLoaded", () => {
    ...
    function getColorInfoByName(name) {
        return COLOR_INFO_MAP.find(c => c.name === name);
    }
    ...
    function drawGridLines(targetCtx, width, height, cell) {
        targetCtx.canvas.width = width * cell;
        targetCtx.canvas.height = height * cell;
        ...
        for (let i = 0; i <= width; i++) {
            ...
            targetCtx.lineTo(i * cell, height * cell);
            targetCtx.stroke();
        }
    ...
    function paintCell(x, y) { const c = Math.floor(x / cellSize); const r = Math.floor(y / cellSize); if (r >= 0 && r < gridHeight && c >= 0 && c < gridWidth) { if (drawingMatrix[r][c] !== currentColorName) { drawingMatrix[r][c] = currentColorName; redrawDrawing(); saveState(); } } }
    ...
    function updateOperationControls() { const op = document.querySelector('input[name="operationType"]:checked').value; const isAddSub = ['optellen', 'aftrekken', 'beide_opt_aft'].includes(op); const isMulDiv = ['vermenigvuldigen', 'delen', 'beide_verm_del'].includes(op); rangeControls.style.display = isAddSub ? 'flex' : 'none'; tablesControls.style.display = isMulDiv ? 'flex' : 'none'; if (isMulDiv) applyAllTablesState(); }
    ...
    function generateProblemForOutcome(desiredOutcome, operationType, options) {
        ...
        switch (operation) {
            ...
            case 'vermenigvuldigen': if (selectedTables.length === 0) return null; const possibleFactors = selectedTables.filter(factor => desiredOutcome % factor === 0 && (desiredOutcome / factor) <= 10); if (possibleFactors.length > 0) { const factor1 = possibleFactors[Math.floor(Math.random() * possibleFactors.length)]; const factor2 = desiredOutcome / factor1; return `${factor1} x ${factor2}`; } return null;
    ...
    function getWorksheetData() {
        ...
        MIN_EXERCISES = Math.min(MIN_EXERCISES, maxOutcomeRange + 1);
        const colorCounts = new Map(); let totalFigureCells = 0;
    ...
    function generateAndDisplayLegend(legendData) {
        ...
        werkbladLegendContent.style.display = 'flex';
        werkbladLegendContent.style.flexDirection = 'row';
        werkbladLegendContent.style.flexWrap = 'wrap';
        werkbladLegendContent.style.alignContent = 'flex-start';
        
        let allExercises = [];
        legendData.forEach((exercises, name) => {
            const colorInfo = getColorInfoByName(name);
            exercises.forEach(exercise => allExercises.push({ ...exercise, colorName: name, colorHex: colorInfo.hex }));
        });
    ...
    function drawWorksheetPreview() {
        ...
        werkbladCtx.fillStyle = 'black';
        const fontSize = Math.max(10, werkbladCellSize * 0.7);
        werkbladCtx.font = `${fontSize}px Arial`;
        ...
        for (let r = 0; r < gridHeight; r++) {
            for (let c = 0; c < gridWidth; c++) {
                const colorName = drawingMatrix[r][c];
                let displayedNumber;
                if (colorName === "Achtergrond") {
                    displayedNumber = distractorNumbers.length > 0 ? distractorNumbers[Math.floor(Math.random() * distractorNumbers.length)] : '';
                } else {
                    const pool = outcomePools.get(colorName);
                    displayedNumber = pool && pool.length > 0 ? pool[Math.floor(Math.random() * pool.length)] : '?';
                }
    ...
    async function generatePdf() {
        ...
        const textFontSize = Math.max(4, Math.min(12, pdfCellSize * 2.8));
    ...
    // --- Werkblad Modal Event Listeners ---
    showPreviewBtn.addEventListener('click', () => {
        drawWorksheetPreview();
        werkbladModal.style.display = 'flex';
    });

    sluitWerkbladBtn.addEventListener('click', () => {
        werkbladModal.style.display = 'none';
    });
