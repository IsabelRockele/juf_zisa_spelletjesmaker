...
(function () {
  ...
  function sorteren(cfg,opl){const ids=categorieen(), groepen=[];for(let i=0;i<ids.length;i+=3)groepen.push(ids.slice(i,i+3));return groepen.map((deel,p)=>{const ws=deel.flatMap(id=>kies(woorden([id]),2,`ov40-${p}-${id}`,opl));const bank=`<div class="ov40-bank">${ws.map(w=>opl?markeer(w):w.tekst).join(" · ")}</div>`;const kolommen=`<div class="ov40-kolommen" style="--kol:${deel.length}">${deel.map(id=>`<div><h3>${LABEL[id]}</h3>${opl?ws.filter(w=>w.categorie===id).map(w=>`<p>${markeer(w)}</p>`).join(""):Array.from({length:3},()=>lijn(cfg)).join("")}</div>`).join("")}</div>`;return pagina(`Kleur en sorteer de hoorwoorden${groepen.length>1?` — deel ${p+1}`:""}`,[`Je oefent woorden met ${soortenTekst(deel)}.`,"Kleur zelf in elk woord het moeilijke woordstuk.","Schrijf elk woord in de juiste kolom."],bank+kolommen,cfg,opl);}).join("");}
  ...
  function bouwen(cfg,opl){const ids=categorieen(),ws=kiesGemengd(ids,6,"ov42",opl);const inhoud=`<div class="ov42-kaarten ov42-twee-kolommen">${ws.map((w,i)=>{const juist=w.tekst.replace(/^hij\s+/i,""),delen=opl?[juist]:husselDelen(w.tekst);return `<div class="ov42-kaart"><b>${i+1}.</b><div class="ov42-delen">${delen.map(d=>`<span>${d}</span>`).join("")}</div>${opl?`<div class="ov40-oplossing">${markeer({...w,tekst:juist})}</div>`:lijn(cfg)}</div>`;}).join("")}</div>`;return pagina("Bouw het hoorwoord",[`Je oefent woorden met ${soortenTekst(ids)}.`,"Zet de letters of lettergroepjes in de juiste volgorde.","Schrijf het volledige woord op de lijn."],inhoud,cfg,opl);}
  ...
  window.SpellingModules.ov42=module("ov42","Hoorwoord bouwen",["basis"],bouwen);
