...
const left      = xLeft;
  ...
  const emoji = emojiLijst[oefIndex % emojiLijst.length];
...
if (cfg.hoofdBewerking === 'tafels-inzicht') {
  ...
  colWidth = kolBreed;
    ...
    function tekenSegmentKader(topY, bottomY, cfg) {
  // standaard kader
  let x = LEFT_PUNCH_MARGIN + 2;
  let w = 210 - (LEFT_PUNCH_MARGIN + 2) - 6;

    // voor tafels-inzicht: maar 12 mm opschuiven, niet 30
  if (cfg && cfg.hoofdBewerking === 'tafels-inzicht') {
    x += 12;
    w -= 12;
  }
  ...
  if (cfg.hoofdBewerking === 'tafels-inzicht') return;  // ✳︎ ook NIET voor tafels-inzicht
...
if (Array.isArray(cfg._oefeningen)) {
  ...
  drawTafelsInzichtPDF(doc, x, y, oef, idx);
        }
...
// --- emoji → PNG (voor PDF) ---
// we cachen, zodat 20 oefeningen niet 20x tekenen
const __TI_EMOJI_CACHE = {};
function __tiEmojiToPng(emoji, sizePx = 30) {
  if (__TI_EMOJI_CACHE[emoji]) return __TI_EMOJI_CACHE[emoji];

  // extra marge rondom de emoji zodat niets wordt afgesneden
  const pad = 6;                         // 6px rondom
  const c = document.createElement('canvas');
  c.width  = sizePx + pad * 2;          // breder
  c.height = sizePx + pad * 2;          // hoger
  const ctx = c.getContext('2d');

  ctx.clearRect(0, 0, c.width, c.height);
  ctx.textBaseline = 'top';
  ctx.font = `${sizePx - 2}px "Apple Color Emoji","Segoe UI Emoji","Noto Color Emoji",system-ui,sans-serif`;

  // NIET op 0,0 tekenen → met pad naar binnen
  ctx.fillText(emoji, pad, pad);

  const url = c.toDataURL('image/png');
  __TI_EMOJI_CACHE[emoji] = url;
  return url;
}


function tekenTafelInzicht(doc, oef, x, y, i = 0) {
  const groepBreedte = 10;
  const marge = 4;

  // ZELFDE reeks als in PDF
 const emojiLijst = [
  '🚗', '🚙',           // auto's
  '⚽', '🏐', '🔵',      // ballen
  '🚲',                 // fietsen
  '🌸',                 // bloemen (in plaats van bouwblokken)
  '📦',                 // dozen/pakjes
  '🍎'                  // appels
];
const woorden = [
  ...
  const groepen  = oef.groepen || oef.tafel || 1;
  const perGroep = (oef.grootte || oef.factor || oef.tafel || 1);

  // kies per oefening hetzelfde indexmechanisme als in PDF
  const idx   = i % emojiLijst.length;
  const woord = woorden[idx];

  for (let g = 0; g < groepen; g++) {
    for (let b = 0; b < perGroep; b++) {
      const px = x + g * (groepBreedte + marge) + b * 2;
      const py = y;
      const emoji = emojiLijst[(g + i) % emojiLijst.length];
      const imgUrl = __tiEmojiToPng(emoji, 32);

      try {
        doc.addImage(imgUrl, 'PNG', px, py - 5.5, 6, 6);
      } catch (e) {
        doc.setFontSize(11);
        doc.text(emoji, px, py);
      }
    }
  }

  // zinnen eronder – nu met dynamisch woord
  const regels = [
    'Ik zie _____ groepen.',
    `In elke groep zie ik _____ ${woord}.`,
    'Dat is _____ + _____ + _____ .',
    'Dat is _____ keer _____ .',
    'Dat is _____ × _____ .'
  ];
