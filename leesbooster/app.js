(() => {
  ...
  const state = { family:'combinaties', vorm:'woorden', selectedGoals:['wr'], blocks:[], nonce:0, storySequence:0 };
  const vormen = [
  ...
  function storySentences(ds,theme){
  ...
  function storyPages(b,startPage,blockIndex){
    ...
    const lines=storySentences(ds,theme); const story=lines.map(s=>b.niveau==='steun'?ds.reduce((out,d)=>markeer(out,d),s):s).join(' ');
    ...
    const page1=`<article class="paper story-page">${remove}${paperHeader('Samen groeien in lezen',b.naamDatum?'Naam: ____________________<br>Datum: ____________________':'')}<h1 class="story-title">${title}</h1><p class="story-instruction">Lees het verhaal rustig en aandachtig.</p><img class="story-image" src="${theme.image}" alt="Illustratie bij ${theme.title}"><div class="story-text">${story}</div><footer class="page-footer"><span>${lines.length} zinnen · ${ds.map(d=>d.label).join(' · ')}</span><span>pagina ${startPage}</span></footer></article>`;
  ...
  function setButtons(on){['#printPdf','#printPdf2','#vernieuw'].forEach(s=>$(s).disabled=!on);$('#blokTeller').textContent=`${state.blocks.length} oefenblok${state.blocks.length===1?'':'ken'}`}
  function add(){
    ...
    const themaKeuze=$('#verhaalThema').value;const themaIds=allowedStoryThemeIds();
    levels.forEach((niveau,levelIndex)=>{const aantal=niveau==='steun'?Math.max(6,gekozenAantal-4):niveau==='basis'?Math.max(8,gekozenAantal-2):gekozenAantal;const verhaalThema=themaKeuze==='verrassing'?themaIds[(state.storySequence+levelIndex)%themaIds.length]:themaKeuze;state.blocks.push({doelen:[...state.selectedGoals],vorm:state.vorm,niveau,aantal,naamDatum:$('#naamDatum').checked,vraagType:$('#vraagType').value,woordenschatType:$('#woordenschatType').value,verhaalThema})});
    if(state.vorm==='verhaal'&&themaKeuze==='verrassing')state.storySequence=(state.storySequence+levels.length)%themaIds.length;
  ...
  const print=()=>window.print();$('#printPdf').addEventListener('click',print);$('#printPdf2').addEventListener('click',print);
