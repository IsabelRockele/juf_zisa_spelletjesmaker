...
/* ═══════════════════════════════════════════════════════════════════════════
   INIT
   ═══════════════════════════════════════════════════════════════════════════ */
window.RekenvierkantShared = {
  normaliseer(input = {}) {
    const formaat = input.formaat === '7x7' ? '7x7' : '5x5';
    let soort = ['optellen','aftrekken','gemengd-plus-min','maal','maal-groot','maaldeel-gemengd'].includes(input.soort) ? input.soort : 'optellen';
    if (formaat === '7x7' && ['maal','maal-groot','maaldeel-gemengd'].includes(soort)) soort = 'optellen';
    return {
      formaat,
      weergave: formaat === '7x7' && input.weergave === 'kader' ? 'kader' : 'klassiek',
      soort,
      niveau: [10,20,100,1000].includes(Number(input.niveau)) ? String(Number(input.niveau)) : '20',
      brug: ['zonder','met','gemengd'].includes(input.brug) ? input.brug : 'gemengd',
      tafelPositie: input.tafelPositie === 'rechts' ? 'rechts' : 'links',
      tafels: Array.isArray(input.tafels) && input.tafels.length >= 2 ? input.tafels.map(Number).filter(n => n >= 1 && n <= 10) : [2,5]
    };
  },
  render(input = {}) {
    const cfg = this.normaliseer(input), cols = cfg.formaat === '7x7' ? 7 : 5, rows = cols;
    const bronCfg = ['optellen','aftrekken','gemengd-plus-min'].includes(cfg.soort)
      ? { soort: cfg.soort, typeOpgave: cfg.soort === 'gemengd-plus-min' ? 'gemengd' : cfg.soort, niveau: cfg.niveau, brug: cfg.brug }
      : { soort: cfg.soort, bewerking: cfg.soort === 'maaldeel-gemengd' ? 'gemengd' : 'maal', maalType: cfg.soort === 'maal-groot' ? 'groot' : 'tafels', tafelPositie: cfg.tafelPositie, tafels: cfg.tafels };
    const resultaat = genereerGrid(cols, rows, bronCfg);
    if (!resultaat.full) throw new Error(resultaat.melding || 'Geen geldig rekenvierkant gevonden.');
    const display = maakDisplayGrid(resultaat.full, cols, rows), canvas = document.createElement('canvas');
    const breedte = cols === 5 ? 310 : 350, kaderHoogte = cfg.weergave === 'kader' ? 105 : 0;
    canvas.width = breedte; canvas.height = breedte + kaderHoogte;
    const tekenContext = canvas.getContext('2d');
    tekenContext.fillStyle = '#f6fafd'; tekenContext.fillRect(0, 0, canvas.width, canvas.height);
    let y = 0;
    if (cfg.weergave === 'kader') {
      const ontbrekend = [];
      for (let r=0;r<rows;r+=2) for (let c=0;c<cols;c+=2) if (display[r][c] === '___') ontbrekend.push(resultaat.full[r][c]);
      y = tekenAntwoordKader(tekenContext, ontbrekend, 0, 0, breedte);
    }
    tekenGrid(tekenContext, display, display, 0, y, breedte / cols, breedte / rows, cols, rows, false);
    return { dataUrl: canvas.toDataURL('image/png'), width: canvas.width, height: canvas.height, config: cfg, melding: resultaat.melding };
  }
};
