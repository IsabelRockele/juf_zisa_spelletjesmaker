...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.getElementById("downloadPdfBtn").addEventListener("click", async ()=>{
    if (currentViewMode!=='template-canvas' || !currentTemplateGrid) { alert("Er is geen template om te downloaden."); return; }
    if (!window.OntdekTrial) { alert('De gratis downloadcontrole is nog niet klaar. Probeer het over enkele seconden opnieuw.'); return; }
    try {
      await window.OntdekTrial.authorizeDownload(1);
    } catch (error) {
      if (error?.code !== 'ONTDEK_LOGIN') alert(error?.message || 'De PDF kon niet worden gedownload.');
      return;
    }
    removeCurrentInput();
    drawPathTemplateOnCanvas(currentTemplateGrid);
    ...
    const M        = 14;   // marge links/rechts
    const beschikB = PW - 2 * M;

    // ── Paginakop ──────────────────────────────────────────────────────────
    let y = M;

    // Naam & Datum invullijnen
    doc.setFontSize(14);
    doc.setFont('helvetica', 'normal');
    doc.setTextColor(26, 45, 64);
    doc.text('Naam:', M, y + 5);
    const naamB = doc.getTextWidth('Naam:') + 2;
    doc.setDrawColor(26, 45, 64);
    doc.setLineWidth(0.5);
    doc.line(M + naamB, y + 5.5, M + 80, y + 5.5);

    doc.text('Datum:', M + 90, y + 5);
    const datumB = doc.getTextWidth('Datum:') + 2;
    doc.line(M + 90 + datumB, y + 5.5, PW - M, y + 5.5);
    ...
    doc.text('Rekenweg', PW / 2, y + 6, { align: 'center' });
    ...
    doc.setDrawColor(11, 79, 153);
    doc.setLineWidth(0.5);
    doc.roundedRect(M, y, beschikB, zinH, 2, 2, 'FD');
    doc.setFontSize(14);
    doc.setFont('helvetica', 'normal');
    doc.setTextColor(26, 45, 64);
    ...
    // ── Canvas afbeelding: bereken de bounding box van gevulde vakjes ──────
    // Zoek min/max rij en kolom met inhoud
    let minR = GRID_ROWS, maxR = 0, minC = GRID_COLS, maxC = 0;
    for (let r = 0; r < GRID_ROWS; r++) {
      for (let c = 0; c < GRID_COLS; c++) {
        if (currentTemplateGrid[r][c] !== null) {
          if (r < minR) minR = r;
          if (r > maxR) maxR = r;
          if (c < minC) minC = c;
          if (c > maxC) maxC = c;
    ...
    // Teken enkel de bounding box van het pad op een offscreen canvas
    const padCols  = maxC - minC + 1;
    const padRijen = maxR - minR + 1;
    const padding  = 1; // extra vakje marge rondom
    const cropCols = padCols + padding * 2;
    const cropRijen = padRijen + padding * 2;

    const offCanvas = document.createElement('canvas');
    offCanvas.width  = cropCols * CELL_SIZE;
    offCanvas.height = cropRijen * CELL_SIZE;
    const offCtx = offCanvas.getContext('2d');
    offCtx.fillStyle = '#ffffff';
    offCtx.fillRect(0, 0, offCanvas.width, offCanvas.height);

    // Teken alleen de vakjes van het pad, verschoven naar de crop-oorsprong
    for (let r = 0; r < GRID_ROWS; r++) {
      for (let c = 0; c < GRID_COLS; c++) {
        const cell = currentTemplateGrid[r][c];
        if (cell === null) continue;
        const x = (c - minC + padding) * CELL_SIZE;
        const yc = (r - minR + padding) * CELL_SIZE;
        // Witte achtergrond + rand
        offCtx.fillStyle = '#ffffff';
        offCtx.fillRect(x, yc, CELL_SIZE, CELL_SIZE);
        offCtx.strokeStyle = '#000000';
        offCtx.lineWidth = 1;
        offCtx.strokeRect(x, yc, CELL_SIZE, CELL_SIZE);
        // Waarde
        if (cell.value) {
          offCtx.fillStyle = '#000000';
          offCtx.font = `bold ${CELL_SIZE * 0.5}px Arial`;
          offCtx.textAlign = 'center';
          offCtx.textBaseline = 'middle';
          offCtx.fillText(cell.value, x + CELL_SIZE / 2, yc + CELL_SIZE / 2);
        }
      }
    }

    const cropURL = offCanvas.toDataURL('image/png');

    // Schaal zodat het past binnen de beschikbare ruimte (tot voettekst)
    const beschikH = PH - y - 20; // 20mm voor voettekst
    const imgRatio = offCanvas.width / offCanvas.height;
    let imgW = beschikB;
    let imgH = imgW / imgRatio;
    if (imgH > beschikH) { imgH = beschikH; imgW = imgH * imgRatio; }
    // Max 160mm breed zodat het niet te groot wordt
    if (imgW > 160) { imgW = 160; imgH = imgW / imgRatio; }

    const xImg = M + (beschikB - imgW) / 2;
    doc.addImage(cropURL, 'PNG', xImg, y, imgW, imgH);

    // ── Voettekst ──────────────────────────────────────────────────────────
    doc.setFontSize(9);
    doc.setFont('helvetica', 'normal');
    doc.setTextColor(130, 150, 170);
    doc.text("Juf Zisa's Spelgenerator  ·  www.jufzisa.be", PW / 2, PH - 8, { align: 'center' });
    doc.setDrawColor(200, 215, 230);
    doc.setLineWidth(0.3);
    doc.line(M, PH - 12, PW - M, PH - 12);

    doc.save('rekenweg_werkblad.pdf');
  });
