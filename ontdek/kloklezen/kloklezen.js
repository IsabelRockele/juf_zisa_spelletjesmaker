...
function genereerTijdenSlim(moeilijkheden, aantal) {
    ...
    document.getElementById("downloadPdfBtn").addEventListener("click", () => {
        ...
        for (let page = 0; page < numPages; page++) {
            ...
            // ── HEADER ──────────────────────────────────────────
            const naamY  = 16;  // rij 1: Naam + Datum
            const titelY = 26;  // rij 2: titel
            const lijnY  = 30;  // lijn net onder de titel
            const headerH = 36; // canvas start op 36 mm

            // Naam links, Datum rechts
            doc.setFontSize(12);
            doc.setFont(undefined, 'normal');
            doc.text("Naam: _______________________", margin, naamY, { align: 'left' });
            doc.text("Datum: _______________", pageWidth - margin, naamY, { align: 'right' });

            // Titel gecentreerd + vet
            const headerText = numPages > 1
                ? `Oefenen op kloklezen  (${page + 1}/${numPages})`
                : "Oefenen op kloklezen";
            doc.setFontSize(14);
            ...
            doc.line(margin, lijnY, pageWidth - margin, lijnY);

            // Canvas direct onder de header (niet verticaal gecentreerd)
            const yPos = headerH;
