...
doc.setTextColor(26, 58, 92);
  ...
  function _tekenRekenrelatiesBlok(blok) {
    const soort=blok.config?.soort||'familie', kolommen=soort==='familie'?3:2, kolB=CW/kolommen, kadW=kolB-5, kadH=soort==='familie'?76:soort==='rooster'?84:92;
    ...
    blok.oefeningen.forEach((o,i)=>{
      ...
      const x=ML+(i%kolommen)*kolB+2;doc.setLineWidth(.2);doc.setDrawColor(170,195,220);doc.setFillColor(255,255,255);if(soort==='familie'){if(i%3<2)doc.line(x+kadW+2.5,y+3,x+kadW+2.5,y+kadH-3);}else doc.roundedRect(x,y,kadW,kadH,2,2,'FD');
      if(soort==='familie'){
        const bw=(kadW-12)/3;
        ...
        o.oplossingen.forEach((s,j)=>{const ly=y+29+j*11;doc.setDrawColor(0,0,0);doc.setLineWidth(.15);doc.line(x+6,ly,x+kadW-6,ly);if(_metAntwoorden){doc.setFont('helvetica','bold');doc.setFontSize(14);doc.setTextColor(0,112,178);doc.text(s,x+7,ly-1);}});
        ...
        const cw=[18,9,18,9,18],rh=[11,8,11,8,11],fw=cw.reduce((a,b)=>a+b,0),fh=rh.reduce((a,b)=>a+b,0),fx=x+(kadW-fw)/2,fy=y+20;
