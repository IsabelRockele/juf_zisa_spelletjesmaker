...
function _getSplitsConfig() {
  ...
  let _paginaTelTimer = null;
  let _laatstePdfPaginaAantal = null;
  function _markeerPdfLengteOnbekend() {
    const status = document.getElementById('ontdek-pagina-status');
    const btnPdf = document.getElementById('btn-pdf');
    clearTimeout(_paginaTelTimer);
    _laatstePdfPaginaAantal = null;
    if (status) {
      status.dataset.status = 'onbekend';
      status.textContent = bundelData.length ? "Pagina's automatisch berekenen…" : 'Voeg oefeningen toe · de limiet geldt alleen in Ontdek';
    }
    if (btnPdf) btnPdf.disabled = true;
    if (bundelData.length) _paginaTelTimer = setTimeout(controleerPdfLengte, 900);
  }
  window.OntdekPagesChanged = _markeerPdfLengteOnbekend;

  async function controleerPdfLengte() {
    if (bundelData.length === 0) return;
    const status = document.getElementById('ontdek-pagina-status');
    const btnPdf = document.getElementById('btn-pdf');
    const titel = document.getElementById('bundel-titel').value.trim() || 'Rekenbundel';
    status.dataset.status = 'bezig';
    status.textContent = "Pagina's automatisch berekenen…";
    try {
      await new Promise(resolve => requestAnimationFrame(() => requestAnimationFrame(resolve)));
      const paginaAantal = await PdfEngine.telPaginas(bundelData, titel, _updatePdfVoortgang);
      _laatstePdfPaginaAantal = paginaAantal;
      const toegestaan = paginaAantal <= 3;
      status.dataset.status = toegestaan ? 'goed' : 'teveel';
      status.textContent = toegestaan
        ? `${paginaAantal} van maximaal 3 pagina${paginaAantal === 1 ? '' : "'s"} · download mogelijk · PRO heeft geen paginalimiet`
        : `${paginaAantal} pagina's · download niet mogelijk in Ontdek (max. 3) · PRO heeft geen paginalimiet`;
      btnPdf.disabled = !toegestaan;
    } catch (fout) {
      console.error('PDF-lengte controleren mislukt:', fout);
      status.dataset.status = 'teveel';
      status.textContent = 'Paginatelling mislukt · wijzig iets om opnieuw te proberen';
  ...
  async function _maakPdf(metAntwoorden) {
    if (bundelData.length === 0) return;
    if (metAntwoorden) {
      alert('Oplossingssleutels zijn uitsluitend beschikbaar in PRO.');
      return;
    }
    const titel = document.getElementById('bundel-titel').value.trim() || 'Rekenbundel';
    const btnPdf = document.getElementById('btn-pdf');
    ...
    try {
      await new Promise(resolve => requestAnimationFrame(() => requestAnimationFrame(resolve)));
      await PdfEngine.genereer(
        bundelData,
        titel,
        false,
        _updatePdfVoortgang,
        async paginaAantal => {
          if (!window.OntdekTrial?.authorizeDownload) throw new Error('De Ontdek-controle is nog niet klaar.');
          return window.OntdekTrial.authorizeDownload(paginaAantal);
        }
      );
      ...
      const ontdekFout = String(fout?.code || '').startsWith('ONTDEK_');
      toonToast(ontdekFout ? `⚠️ ${fout.message}` : '❌ De PDF kon niet worden gemaakt. Probeer het opnieuw.', '#E74C3C');
    } finally {
      _verbergPdfVoortgang();
      btnPdf.disabled = bundelData.length === 0 || _laatstePdfPaginaAantal === null || _laatstePdfPaginaAantal > 3;
  ...
  return {
    ...
    genereerPreview, controleerPdfLengte, downloadPDF, downloadSleutel,
