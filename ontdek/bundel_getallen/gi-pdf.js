...
window.GI_Pdf = (() => {
  ...
  async function maakPdf(bestandsnaam = 'werkblad-getalinzicht.pdf', opties = {}) {
    ...
    try {
      ...
      if (opties.countOnly) return { pages: plakjes.length };
      ...
      if (!opties.solutions && window.OntdekTrial?.authorizeDownload) {
        await window.OntdekTrial.authorizeDownload(plakjes.length);
      }
      pdf.save(bestandsnaam);
      return { pages: plakjes.length };
