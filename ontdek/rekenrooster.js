document.addEventListener('DOMContentLoaded',()=>{
  ...
  async function downloadPdf(metOplossingen=false,opties={}){
    ...
    if(opties.countOnly){if(toonOplossingen!==vorigeWeergave){toonOplossingen=vorigeWeergave;teken()}return{pages}}
    if(!metOplossingen&&window.OntdekTrial?.authorizeDownload)await window.OntdekTrial.authorizeDownload(pages);
  ...
  window.RekenroosterPdfEngine={countPages:()=>downloadPdf(false,{countOnly:true})};
