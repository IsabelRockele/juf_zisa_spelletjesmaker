...
ensureSpace(leadNeeded + 8);
  ...
  async function generate(options = {}) {
    ...
    if (options.countOnly) return { pages };
    if (window.OntdekTrial?.authorizeDownload) await window.OntdekTrial.authorizeDownload(pages);
...
window.GeldPdfEngine = pdfEngine;
