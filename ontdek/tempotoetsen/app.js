...
function updatePreview() {
  ...
  oefeningen.forEach((o, i) => {
    ...
    if (typeof o.vraag === 'string') {
      ...
      vraag = window.ISPRO ? `getalbeeld (${o.antwoord})` : 'getalbeeld';
    } else {
      vraag = '?';
    }
...
async function genereerPdf({ titel, oefeningen, modus }) {
  ...
  if (window.ISPRO && (modus === 'papier' || modus === 'antwoordblad')) {
    doc.addPage();
  ...
  if (!window.ISPRO && window.OntdekTrial?.authorizeDownload) {
...
async function genereerWeekbladPdf(groepen, weekVan, isHuistaak) {
  const { jsPDF } = window.jspdf;
  ...
  if (window.ISPRO) groepen.forEach((dagenOpBlad, bladIdx) => {
