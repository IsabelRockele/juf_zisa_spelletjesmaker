...
const PdfEngine = (() => {
  ...
  // ── Hoofdfunctie ─────────────────────────────────────────────────────────
  async function genereer(piramides, opties = {}) {
    ...
    if (opties.countOnly) return { pages };
    if (!toonOplossing) {
      if (!window.OntdekTrial) { alert('De gratis downloadcontrole is nog niet klaar. Probeer het over enkele seconden opnieuw.'); return { pages, saved:false }; }
      try {
        await window.OntdekTrial.authorizeDownload(pages);
      } catch (error) {
        if (error?.code !== 'ONTDEK_LOGIN') alert(error?.message || 'De PDF kon niet worden gedownload.');
        return { pages, saved:false };
      }
    }
    doc.save(bestandsnaam);
    return { pages, saved:true };
  }

  return { genereer };

})();
window.RekenpiramidePdfEngine = PdfEngine;
