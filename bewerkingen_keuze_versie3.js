...
if (typeof window !== 'undefined') {
  ...
  maakSplitsBtn.addEventListener('click', () => {
    ...
    window.location.href = 'bewerkingen_werkblad_versie3.html';
  });
}
...
const vrij = (document.getElementById(veldId) || {}).value?.trim();
  ...
  // ====== TAFELS – INZICHT ======
const tiMelding     = document.getElementById('tafelsInzichtMelding');
const tiMaakBtn     = document.getElementById('ti_maakBtn');
const tiVoegToeBtn  = document.getElementById('ti_voegToeBtn');
const tiSelectAll   = document.getElementById('ti_selectAll');

// “alle tafels” aanvinkactie
if (tiSelectAll) {
  tiSelectAll.addEventListener('change', (e) => {
    const all = document.querySelectorAll('input[name="ti_tafel"]');
    all.forEach(cb => cb.checked = e.target.checked);
  });
}

// Lees instellingen uit de UI
function bouwTafelsInzichtSettings() {
  const gekozen = Array.from(document.querySelectorAll('input[name="ti_tafel"]:checked'))
...
const maxUitkomst = (bereikKeuze === 'volledig') ? 'volledig' : 12;
  const aantal = Math.max(1, parseInt((document.getElementById('ti_aantal') || { value: 8 }).value, 10) || 8);
  const opdracht = (document.getElementById('ti_opdracht') || {}).value?.trim() || '';

 return {
  segmentId: nieuwSegmentId(),
  hoofdBewerking: 'tafels-inzicht',
  gekozenTafels: gekozen,
  tafelType: type,
  bereik: bereikKeuze,
...
// “Maak werkblad”
if (tiMaakBtn) {
  tiMaakBtn.addEventListener('click', () => {
    if (tiMelding) tiMelding.textContent = '';
    const s = bouwTafelsInzichtSettings();
    if (!s.gekozenTafels.length) {
      if (tiMelding) tiMelding.textContent = 'Kies minstens één tafel.';
      return;
    }
    localStorage.setItem('werkbladSettings', JSON.stringify(s));
    window.location.href = 'bewerkingen_werkblad_versie3.html';
  });
}
...
// “Voeg toe aan bundel”
if (tiVoegToeBtn) {
  tiVoegToeBtn.addEventListener('click', () => {
    if (tiMelding) tiMelding.textContent = '';
    const s = bouwTafelsInzichtSettings();
    if (!s.gekozenTafels.length) {
      if (tiMelding) tiMelding.textContent = 'Kies minstens één tafel.';
    ...
    const bereikTxt = (s.bereik === 'volledig') ? 'volledige tafel' : '12';
const sub = `tafels ${s.gekozenTafels.join(', ')} • ${s.tafelType} • tot ${bereikTxt} • aant. ${s.numOefeningen}`;
    bundelPush({
      titel: 'Tafels – inzicht',
      subtitle: sub,
      settings: s
    });
  });
